...
export default () => {
    ...
    for(let instruction of instructionList){
        ...
        oldSantaPos.push([santaPos]);
    }

    for(let i=0 ; i < oldSantaPos.length; i++) {
        const [firstPosX, firstPosY] = oldSantaPos[i]
        if(result === null) {
            for (let j=1; j < oldSantaPos.length; i++){
                console.log(`oldSantaPos[i] is ${oldSantaPos[i]} and oldSantaPos[j] is ${oldSantaPos[j]}`)
                const [secondPosX, secondPosY] = oldSantaPos[j]
                if(firstPosX === secondPosX && firstPosY === secondPosY) {
                    result = oldSantaPos[j];
                    break
                }
            }
        } else {
