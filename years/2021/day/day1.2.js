...
/*
    Link Here :https://adventofcode.com/2021/day/1
...
export default () => {
    const deeps = fs.readFileSync(path.join(process.cwd(), "./years/2021/in/day1.txt"), {encoding: "utf8"})
        .split(`\r\n`)
        .map(nb => parseInt(nb))
        ...
        return deeps.reduce((acc, curr, index, array) => {
            //Each turn I calculate the sum of the current value(n) + value(n+1) + value(n+2)
            const sumOfThree = (curr + array[index + 1] + array[index + 2])
            //if last sum is not undefined so it's not the first turn
            if (lastSum) {
                //if the sum of Three is superior of lasSum
                if (sumOfThree > lastSum) {
                    acc++
                }
            }
            //at each ending turn I save the sumOfthree as the lastSum for the next turn
            lastSum = sumOfThree
